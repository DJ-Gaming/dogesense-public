...
void vec3_t_3d_multiply_position(const view_matrix_t& src1, const vec3_t& src2, vec3_t& dst) {
...
void view_matrix_t::get_basis_vec3_t_3d(vec3_t& forward, vec3_t& left, vec3_t& up) const {
	forward.init(m[0][0], m[1][0], m[2][0]);
	left.init(m[0][1], m[1][1], m[2][1]);
	up.init(m[0][2], m[1][2], m[2][2]);
}

void view_matrix_t::set_basis_vec3_t_3d(const vec3_t& forward, const vec3_t& left, const vec3_t& up) {
	set_forward(forward);
	set_left(left);
	set_up(up);
}
...
void view_matrix_t::pre_translate(const vec3_t& trans) {
	vec3_t tmp;
	vec3_t_3d_multiply_position(*this, trans, tmp);
	m[0][3] = tmp.x;
	m[1][3] = tmp.y;
	m[2][3] = tmp.z;
}
...
vec3_t view_matrix_t::vec3_t_3d_multiply(const vec3_t& vec) const {
	vec3_t result;
	vec3_t_3d_multiply_position(*this, vec, result);
	return result;
}

vec3_t view_matrix_t::vec3_t_3d_transpose(const vec3_t& vec) const {
	vec3_t tmp = vec;
	tmp.x -= m[0][3];
	tmp.y -= m[1][3];
	tmp.z -= m[2][3];
...
vec3_t view_matrix_t::vec3_t_3d_multiply_upper(const vec3_t & vec) const {
	return vec3_t(
...
vec3_t view_matrix_t::vec3_t_3d_transpose_rotation(const vec3_t & vec) const {
	return vec3_t(
...
void view_matrix_t::vec3_t_3d_multiply(const vec3_t & in, vec3_t & out) const {
	vec_t rw;
