...
public:
	...
	void		get_basis_vec3_t_3d(vec3_t& forward, vec3_t& left, vec3_t& up) const;
	void		set_basis_vec3_t_3d(const vec3_t& forward, const vec3_t& left, const vec3_t& up);
...
public:
	...
	void		vec3_t_3d_multiply(const vec3_t & in, vec3_t & out) const;
	...
	vec3_t		vec3_t_3d_multiply(const vec3_t & vec) const;

	// Apply the inverse (transposed) rotation (only works on pure rotation matrix)
	vec3_t		vec3_t_3d_transpose_rotation(const vec3_t & vec) const;

	// Multiply by the upper 3 rows.
	vec3_t		vec3_t_3d_multiply_upper(const vec3_t & vec) const;

	// Apply the inverse (transposed) transformation (only works on pure rotation/translation)
	vec3_t		vec3_t_3d_transpose(const vec3_t & vec) const;
...
public:
	...
	view_matrix_t		normalize_basis_vec3_t_3d() const;
